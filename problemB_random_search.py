...
# problem C----------------------------------
# steplength paramter alpha 의 크기를 서로 다른 2개의 값을 사용하라
# 그에 따라 output에 미치는 영향 기술
...
for i in range(z):
    ...
    for j in range(n_directions):
        ...
        direction_a = direction * 1 # 방향벡터 d
        direction_b = direction * 2 # 2d

        # short step size 방향으로 이동한 후보점
        x_candidate_a = x_current + alpha * direction_a
        f_candidate_a = cost_function(x_candidate_a)

        candidates.append((f_candidate_a, x_candidate_a))

        # long step size 방향으로 이동한 후보점
        x_candidate_b = x_current + alpha * direction_b
        f_candidate_b = cost_function(x_candidate_b)

        candidates.append((f_candidate_b, x_candidate_b))
...
# 축 라벨 및 제목
ax.set_title('Machine Learning Problem B')
ax.set_xlabel('x')
ax.set_ylabel('y')
